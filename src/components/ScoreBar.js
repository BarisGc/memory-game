...
function ScoreBar() {
    ...
    return (
        ...
        <Row xs={1} md={3} className='scoreBar justify-content-center align-items-center mt-0 mb-2 py-2 h-100 '>
            ...
            {isCompleted ? <Col className='score text-center ' onClick={() => setIsShuffle(!isShuffle ? true : false)}>New Game</Col> : <Col className='score text-center '>{`${numberOFUnOpenCards} Cards Left`}</Col>}
        </Row>
    )
}
